...
fn remove_from_cart(inventory: &mut Vec<Device>, cart: &mut Vec<Device>) {
    println!("Your cart:");
    list_devices(cart);
    let choice: u32 = read_line("Enter the device number to remove:")
        .parse()
        .unwrap();
    ...
    let quantity: u32 = read_line(
        format!(
            "Please select quantity of {} to remove from your cart. Available units: {}",
    ...
    println!(
        "{} units of {} removed from your cart",
...
fn add_to_cart(inventory: &mut Vec<Device>, cart: &mut Vec<Device>) {
    ...
    let quantity: u32 = read_line(
        format!(
            "Please select quantity of {} to add to your cart. Available units: {}",
            device.name, device.quantity
        )
        .as_str(),
    )
...
fn list_devices(inventory: &Vec<Device>) {
    ...
    for device in inventory {
        println!(
            "{}. {} - ${:.2} - {}",
            i, device.name, device.price, device.quantity
        );
        i += 1;
    }
}
