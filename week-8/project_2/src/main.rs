...
fn add_candidate(candidates: &mut Vec<Candidate>) {
    ...
    let age: u32 = read_line("Enter age:")
        .parse()
        .expect("Failed to parse age");
    let experience: u32 = read_line("Enter years of experience:")
        .parse()
        .expect("Failed to parse experience");
    candidates.push((name, age, experience));
    println!("Successfully added candidate");
}

fn remove_candidate(candidates: &mut Vec<Candidate>) {
    let remove_no: usize = read_line("Enter candidate number to remove:")
        .parse()
        .expect("Failed to parse candidate number");

    let removed = candidates.remove(remove_no - 1);

    println!("Removed candidate {remove_no}: {}", removed.0);
}
